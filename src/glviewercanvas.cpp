...
void GLViewerCanvas::render( wxPaintEvent& evt )
{
  ...
  view  = glm::translate(
    view,
    glm::vec3( 0.0f, 0.0f, -1.0f ) ); // PLACEHOLDER: note that we're translating the scene in the reverse direction of where we want to move
  proj = glm::perspective( glm::radians( 45.0f ), ratio_w_h, 0.1f, 200.0f ); // FOV, ratio, znear, zfar
...
void GLViewerCanvas::LoadShadersGL()
{
  ...
  if( !success )
  {
    ...
    std::cout << "\t --> [1/3] Vrtx Shader compiled successfully!" << std::endl;
  ...
  if( !success )
  {
    ...
    std::cout << "\t --> [2/3] Frag Shader compiled successfully!" << std::endl;
  ...
  if( !success )
  {
    ...
    std::cout << "\t --> [3/3] Shader linked successfully!" << std::endl << std::endl;
  ...
  // shaderProgram = LoadShaders( "resources/SimpleVertexShader.vertexshader", "resources/SimpleFragmentShader.fragmentshader" ); // NO FILE NOR
  // FUNC!!!
  ...
  else
  {
    ...
    if( stackedRacks )
    {
      vtxData.insert( vtxData.end(),
                      { -nodeSize + 2.2f * nodeSep * i * nodeSize, nodeSize - j * nodeSep, -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx } );
      vtxData.insert( vtxData.end(),
                      { -nodeSize + 2.2f * nodeSep * i * nodeSize, nodeSize - j * nodeSep, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx } );
      vtxData.insert( vtxData.end(),
                      { nodeSize + 2.2f * nodeSep * i * nodeSize, nodeSize - j * nodeSep, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx } );
    ...
    {
      vtxData.insert( vtxData.end(),
                      { -nodeSize + 2.2f * nodeSep * j * nodeSize,
                        nodeSize,
                        -nodeSize - N * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * i * nodeSize } );
      vtxData.insert(
        vtxData.end(),
        { -nodeSize + 2.2f * nodeSep * j * nodeSize, nodeSize, nodeSize - N * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * i * nodeSize } );
      vtxData.insert(
        vtxData.end(),
        { nodeSize + 2.2f * nodeSep * j * nodeSize, nodeSize, nodeSize - N * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * i * nodeSize } );
      vtxData.insert(
  ...
  for( int i = 0; i < N; ++i )
  {
    for( int j = 0; j < M; ++j )
    {
      ...
      if( stackedRacks ) // fix rack > 2 coordinates...
      {
        ...
        // glNormal3f( 0.0f, 1.0f, 0.0f );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X0 * 2.0f * ( nodeSize ),
                          nodeSize - nodeIdx * nodeSep + val,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y0 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X0 * 2.0f * ( nodeSize ),
                          nodeSize - nodeIdx * nodeSep + val,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X1 * 2.0f * ( nodeSize ),
                          nodeSize - nodeIdx * nodeSep + val,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X1 * 2.0f * ( nodeSize ),
      ...
      else
      {
        ...
        // glNormal3f( 0.0f, 1.0f, 0.0f );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X0 * 2.0f * ( nodeSize ),
                          val + nodeSize,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y0 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X0 * 2.0f * ( nodeSize ),
                          ...
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X1 * 2.0f * ( nodeSize ),
                          val + nodeSize,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X1 * 2.0f * ( nodeSize ),
  ...
  for( int i = 0; i < N; ++i )
  {
    for( int j = 0; j < M; ++j )
    {
      ...
      if( stackedRacks ) // fix rack > 2 coordinates...
      {
        ...
        // glNormal3f( 0.0f, 1.0f, 0.0f );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X0 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
                          nodeSize - nodeIdxJ * nodeSep + val,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y0 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X0 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
                          nodeSize - nodeIdxJ * nodeSep + val,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X1 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
                          nodeSize - nodeIdxJ * nodeSep + val,
                          -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert( vtxData.end(),
                        { -nodeSize + X1 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
      ...
      else
      {
        ...
        vtxData.insert(
          vtxData.end(),
          { -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X0 * 2.0f * ( nodeSize ),
            val + nodeSize,
            -nodeSize - Nloc * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * nodeIdxI * nodeSize + Y0 * 2.0f * ( nodeSize ) } );
        vtxData.insert(
          vtxData.end(),
          { -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X0 * 2.0f * ( nodeSize ),
            val + nodeSize,
            -nodeSize - Nloc * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * nodeIdxI * nodeSize + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert(
          vtxData.end(),
          { -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X1 * 2.0f * ( nodeSize ),
            val + nodeSize,
            -nodeSize - Nloc * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * nodeIdxI * nodeSize + Y1 * 2.0f * ( nodeSize ) } );
        vtxData.insert(
          vtxData.end(),
...
void GLViewerCanvas::DrawRack( int rackIdx, int pir, int rackSize ) // pir = "pos in rack"
{
  ...
  if( !groupRacks )
  {
    ...
    {
      glTexCoord2f( 0, 0 );
      glVertex3f( -nodeSize, nodeSize - pir * nodeSep, -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
      glTexCoord2f( 0, 1 );
      glVertex3f( -nodeSize, nodeSize - pir * nodeSep, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
      glTexCoord2f( 1, 1 );
      glVertex3f( nodeSize, nodeSize - pir * nodeSep, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
    ...
    {
      glTexCoord2f( 0, 0 );
      glVertex3f( -nodeSize + 2.2f * nodeSep * pir * nodeSize, nodeSize, -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
      glTexCoord2f( 0, 1 );
      glVertex3f( -nodeSize + 2.2f * nodeSep * pir * nodeSize, nodeSize, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
      glTexCoord2f( 1, 1 );
      glVertex3f( nodeSize + 2.2f * nodeSep * pir * nodeSize, nodeSize, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
  ...
  else
  {
    ...
    {
      glTexCoord2f( 0, 0 );
      glVertex3f( -nodeSize + 2.2f * nodeSep * i * nodeSize, nodeSize - j * nodeSep, -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
      glTexCoord2f( 0, 1 );
      glVertex3f( -nodeSize + 2.2f * nodeSep * i * nodeSize, nodeSize - j * nodeSep, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
      glTexCoord2f( 1, 1 );
      glVertex3f( nodeSize + 2.2f * nodeSep * i * nodeSize, nodeSize - j * nodeSep, nodeSize - 3.3f * nodeSep * nodeSize * rackIdx );
    ...
    {
      glTexCoord2f( 0, 0 );
      glVertex3f( -nodeSize + 2.2f * nodeSep * j * nodeSize,
                  nodeSize,
                  -nodeSize - N * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * i * nodeSize );
      glTexCoord2f( 0, 1 );
      glVertex3f( -nodeSize + 2.2f * nodeSep * j * nodeSize,
  ...
  for( int i = 0; i < N; ++i )
  {
    for( int j = 0; j < M; ++j )
    {
      ...
      if( stackedRacks ) // fix rack > 2 coordinates...
      {
        ...
        glNormal3f( 0.0f, 1.0f, 0.0f );
        glTexCoord2f( X0, Y0 );
        glVertex3f( -nodeSize + X0 * 2.0f * ( nodeSize ),
                    nodeSize - nodeIdx * nodeSep + val,
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y0 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X0, Y1 );
        glVertex3f( -nodeSize + X0 * 2.0f * ( nodeSize ),
                    ...
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y1 );
        glVertex3f( -nodeSize + X1 * 2.0f * ( nodeSize ),
                    nodeSize - nodeIdx * nodeSep + val,
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y0 );
        glVertex3f( -nodeSize + X1 * 2.0f * ( nodeSize ),
      ...
      else
      {
        ...
        glNormal3f( 0.0f, 1.0f, 0.0f );
        glTexCoord2f( X0, Y0 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X0 * 2.0f * ( nodeSize ),
                    val + nodeSize,
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y0 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X0, Y1 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X0 * 2.0f * ( nodeSize ),
                    ...
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y1 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X1 * 2.0f * ( nodeSize ),
                    val + nodeSize,
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y0 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdx * nodeSize + X1 * 2.0f * ( nodeSize ),
  ...
  for( int i = 0; i < N; ++i )
  {
    for( int j = 0; j < M; ++j )
    {
      ...
      if( stackedRacks ) // fix rack > 2 coordinates...
      {
        ...
        glNormal3f( 0.0f, 1.0f, 0.0f );
        glTexCoord2f( X0, Y0 );
        glVertex3f( -nodeSize + X0 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
                    nodeSize - nodeIdxJ * nodeSep + val,
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y0 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X0, Y1 );
        glVertex3f( -nodeSize + X0 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
                    ...
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y1 );
        glVertex3f( -nodeSize + X1 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
                    nodeSize - nodeIdxJ * nodeSep + val,
                    -nodeSize - 3.3f * nodeSep * nodeSize * rackIdx + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y0 );
        glVertex3f( -nodeSize + X1 * 2.0f * ( nodeSize ) + 2.2f * nodeSep * nodeIdxI * nodeSize,
      ...
      else
      {
        ...
        glNormal3f( 0.0f, 1.0f, 0.0f );
        glTexCoord2f( X0, Y0 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X0 * 2.0f * ( nodeSize ),
                    val + nodeSize,
                    -nodeSize - Nloc * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * nodeIdxI * nodeSize + Y0 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X0, Y1 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X0 * 2.0f * ( nodeSize ),
                    ...
                    -nodeSize - Nloc * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * nodeIdxI * nodeSize + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y1 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X1 * 2.0f * ( nodeSize ),
                    val + nodeSize,
                    -nodeSize - Nloc * 3.3f * nodeSep * nodeSize * rackIdx + 2.2f * nodeSep * nodeIdxI * nodeSize + Y1 * 2.0f * ( nodeSize ) );
        glTexCoord2f( X1, Y0 );
        glVertex3f( -nodeSize + 2.2f * nodeSep * nodeIdxJ * nodeSize + X1 * 2.0f * ( nodeSize ),
...
int GLViewerCanvas::setTextureResolution()
{
  ...
  if( coreToNode != 7 && coreToNode <= 16 )
    return 256;
  else if( coreToNode <= 49 )
    return 512;
