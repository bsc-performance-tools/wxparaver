...
wxCmdLineEntryDesc wxparaverApp::argumentsParseSyntax[] = {
  { wxCMD_LINE_SWITCH, "v", "version", "Show wxparaver version." },
  ...
  { wxCMD_LINE_OPTION, "e", "event", "Event type to code linking.", wxCMD_LINE_VAL_NUMBER, wxCMD_LINE_PARAM_OPTIONAL },
  ...
  { wxCMD_LINE_SWITCH, "gtf", "generate_tutorials_file", "", wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL },
...
bool wxparaverApp::OnInit()
{
  ...
  {
    wxMessageBox( wxT( "Error while loading preferences. Using default options." ), wxT( "Preferences error" ), wxOK | wxICON_ERROR );
  }
  ...
  if( !ParaverConfig::getInstance()->getGlobalSingleInstance() )
  {
    ...
    const wxString serviceFullName = wxT( "wxparaver_service-" ) + wxGetUserId() + wxT( "-" ) + wxString::Format( wxT( "%i" ), getpid() );
#else
    ...
    if( m_server->Create( serviceFullName ) )
    {
      ...
      if( wxd.IsOpened() )
      {
        ...
        while( cont )
        {
          ...
          wxString serviceFileName = wxT( "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\" ) + service;
  ...
  else // if( ParaverConfig::getInstance()->getGlobalSingleInstance() )
  {
    ...
    else
    {
      ...
      {
        wxMessageBox( wxT( "Sorry, the existing instance may be too busy to respond." ), wxT( "wxparaver" ), wxICON_INFORMATION | wxOK );
        delete client;
        return false;
      }
    }
  ...
  wxSize mainWindowSize( ParaverConfig::getInstance()->getMainWindowWidth(), ParaverConfig::getInstance()->getMainWindowHeight() );

  mainWindow = new paraverMain( nullptr, SYMBOL_PARAVERMAIN_IDNAME, SYMBOL_PARAVERMAIN_TITLE, SYMBOL_PARAVERMAIN_POSITION, mainWindowSize );
  ...
  if( paraverCommandLineParser.Found( wxT( "i" ) ) )
  {
    ...
    for( unsigned int i = 0; i < paraverCommandLineParser.GetParamCount(); ++i )
    {
      ...
      else if( CFGLoader::isCFGFile( fileName ) && currentTrace != nullptr )
      {
        ...
        if( CFGLoader::loadCFG( mainWindow->GetLocalKernel(), fileName, currentTrace, newWindows, newHistograms, options ) )
        {
          if( !newHistograms.empty() )
          {
            ...
            string composedName = histo->getName() + " @ " + histo->getControlWindow()->getTrace()->getTraceName();

            gHistogram *tmpGHisto = new gHistogram( mainWindow, wxID_ANY, wxString::FromUTF8( composedName.c_str() ) );
          ...
          else
          {
            ...
            gTimeline *tmpTimeline = new gTimeline( mainWindow, wxID_ANY, wxString::FromUTF8( composedName.c_str() ), tmpPos );
  ...
  if( paraverCommandLineParser.Found( wxT( "t" ), &tmpTutorial ) )
  {
    if( mainWindow->GetTutorialsWindow() == nullptr )
    {
      mainWindow->SetTutorialsWindow(
        HelpContents::createObject( TContents::TUTORIAL,
                                    mainWindow,
